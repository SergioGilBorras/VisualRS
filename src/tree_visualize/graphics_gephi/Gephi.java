 ...
 */
package tree_visualize.graphics_gephi;
...
import static tree_visualize.graphics_gephi.Gephi.Type_File_Export.*;
...
public class Gephi {
     ...
     * null
     */
    public Gephi(Maximum_spanning_tree_matrix MRM) throws Exception {
        this.correlation_name = MRM.getSimilarity_measure_name();
        this.file_name = MRM.getFile_name();
        prim_matrix = MRM.getMaximum_spanning_tree_matrix();
     ...
     */
    public void execute(Type_File_Export TFE) throws Exception {
        this.TFE = TFE;
        System.out.println("--:: Grafico " + correlation_name + " to " + TFE.name() + "::--");
     ...
     * @throws Exception
     */
    private void save_gephi_csv() {

        File sFile = new File("./data/" + file_name + "/" + correlation_name + "/gephi_" + correlation_name + ".csv");
        try {
            BufferedWriter bw = new BufferedWriter(new FileWriter(sFile));
            boolean entra = false;
            for (int item = 0; item < prim_matrix.columns; item++) {
                entra = false;
                double[] ColumItem = prim_matrix.getColumn(item).toArray();
                for (int item1 = 0; item1 < prim_matrix.rows; item1++) {
     ...
     * @throws Exception
     */
    private void save_gephi_grafico() throws Exception {
        if (undirectedGraph == null) {
            ...
            for (int i = 0; i < prim_matrix.columns; i++) {
                //Create  nodes
            ...
            for (int i = 0; i < prim_matrix.columns; i++) {
                for (int ii = i + 1; ii < prim_matrix.rows; ii++) {
                    double value = prim_matrix.get(i, ii);
                    if (value >= -1) {
                        value = 1;
...
//Export
        ...
        try {
            if (TFE == PDF) {
                ec.exportFile(new File("./data/" + file_name + "/" + correlation_name + "/gephi_" + correlation_name + ".pdf"));
            } else if (TFE == PNG) {
                ec.exportFile(new File("./data/" + file_name + "/" + correlation_name + "/gephi_" + correlation_name + ".png"));
            } else if (TFE == SVG) {
                ec.exportFile(new File("./data/" + file_name + "/" + correlation_name + "/gephi_" + correlation_name + ".svg"));
            }
